export const Vidster = (elem, sources, width, height) => {
	if (!elem) return;
 ...
 //  var randsrc = sources.randomizr();

  // if (sources) {
  // 	for (var i = 0; i < elem.childNodes.length; i++) {
  // 		var srces = elem.childNodes[i].src.replace(/\/([^\/]+)\.[a-zA-Z0-9]{3,}$/g, randsrc);
  // 		console.log(srces);
  // 	};
  // }
  ...
  // }
	...
	/**
	 * Size the video according to parent element
	 */
	...
	let nativeWidth = width || 854;
	let nativeHeight = height || 480;
	...
	const supportsVideo = (typeof(elem.canPlayType) != 'undefined') ? true : false;
 ...
 * }
 *
 * var sources = elem.childNodes;
 *
 * for (var i = 0; i < sources.length; i++) {
 * 	console.log(sources[i].src);
 * 	var last = sources[i].src.match(/\/([^\/]+)\.[a-zA-Z0-9]{3,}$/)[1];
 * 	console.log(last);
 * };
 *
 * function loop(arr) {
